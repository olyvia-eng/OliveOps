...
function estimate(areaCount = 1, descriptionsPerArea = 3) {
  return {
    ...
    workAreas: Array.from({ length: areaCount }, (_, areaIndex) => ({
      id: `area-${areaIndex}`, name: `Work Area ${areaIndex + 1}`, description: '', sortOrder: areaIndex,
      lineItems: Array.from({ length: descriptionsPerArea }, (_, lineIndex) => ({
        category: lineIndex % 2 ? 'equipment' : 'labour',
        itemName: lineIndex % 2 ? 'Excavator' : 'Labour',
        description: `Complete customer scope item ${areaIndex + 1}.${lineIndex + 1} with a detailed description that wraps safely across the printable proposal page.`,
...
test('proposal PDF renders customer-safe scope, branding, exact projected totals, and acceptance', () => {
  ...
  for (const hidden of ['unitCost', 'sellPrice', 'overheadRecovery', 'estimatedProfit', 'margin', 'Excavator', 'Labour', 'Generated:']) {
