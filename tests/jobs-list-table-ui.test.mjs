...
test('Jobs index uses the compact Estimates-style table instead of record cards', () => {
  assert.match(jobsSource, /<div className="overflow-x-auto">\s*<table className="w-full table-fixed text-sm">/);
  for (const column of ['Job', 'Customer', 'Work Areas', 'Status', 'Actions']) {
    assert.match(jobsSource, new RegExp(`>${column}<`));
  }
  assert.doesNotMatch(jobsSource, />Labour Hours<|>Contract Value</);
...
test('Jobs rows preserve customer and work-area presentation without list-only labour calculations', () => {
  ...
  assert.doesNotMatch(jobsSource, /calculateJobPerformance|performance\.labour|timeEntries|timeCorrections|labourClasses/);
...
test('Jobs list avoids performance and drawer-only risk calculations', () => {
  assert.doesNotMatch(jobsSource, /jobPerformanceById|calculateJobPerformance/);
...
test('Jobs toolbar and table retain responsive and dark-mode treatments', () => {
  ...
  assert.doesNotMatch(jobsSource, /min-w-\[1120px\]/);
});
