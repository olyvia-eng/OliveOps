...
test('customer proposal uses the OliveOps palette and rounded document hierarchy', () => {
  ...
  assert.match(pdfRenderer, /const OLIVE_DEEP = \[74, 100, 24\]/);
  assert.match(pdfRenderer, /const OLIVE = \[107, 142, 35\]/);
  assert.match(pdfRenderer, /const OLIVE_TINT = \[238, 244, 227\]/);
  assert.match(pdfRenderer, /const OLIVE_BORDER = \[202, 223, 162\]/);
  assert.match(pdfRenderer, /doc\.roundedRect\(24, 18, PAGE_WIDTH - 48, 92, 12, 12, 'F'\)/);
});