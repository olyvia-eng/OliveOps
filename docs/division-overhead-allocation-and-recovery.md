...
`Budget.targetMarginPct` is the only persisted Target Net Profit value. Percentage mode edits that value directly. Dollar mode derives it as `target profit dollars / (target profit dollars + planned costs)`; switching display modes does not persist another value. Pricing reads the same `Budget.targetMarginPct` and continues to use margin division rather than markup.
