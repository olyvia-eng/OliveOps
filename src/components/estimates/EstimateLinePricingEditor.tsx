...
import { Button, DecimalTextInput, Input } from '../ui';
  ...
  return <div className="fixed inset-0 z-50">
    ...
    <aside className="absolute inset-y-0 right-0 flex w-full max-w-md flex-col bg-white shadow-2xl dark:bg-brand-800" aria-label="Edit line pricing">
      <div className="flex items-center justify-between border-b border-brand-100 px-4 py-3 dark:border-brand-600">
        <div className="min-w-0"><h2 className="truncate text-sm font-semibold text-gray-900 dark:text-brand-50">{title ?? `Pricing: ${lineItem.itemName || lineItem.description || 'Item'}`}</h2><p className="text-xs text-gray-500 dark:text-brand-300">Estimate only · {lineItem.itemName || lineItem.description || 'Resource'} · {lineItem.unit}</p></div>
      ...
      <div className="flex-1 space-y-6 overflow-y-auto p-4">
        ...
        {initialMode === 'profit' || editServiceUsage ? <label className="block text-sm font-medium text-gray-700 dark:text-brand-100">Profit margin<DecimalTextInput aria-label="Profit margin" displayDecimals={2} value={targetMarginPct} onValueChange={setTargetMarginPct} className="mt-2 h-11 w-full rounded-md border border-brand-100 bg-white px-3 text-right text-base font-semibold text-brand-900 focus:outline-none focus:ring-2 focus:ring-accent-500/40 dark:border-brand-600 dark:bg-brand-700 dark:text-brand-50" /></label>
          : <label className="block text-sm font-medium text-gray-700 dark:text-brand-100">Estimate Price / {lineItem.unit}<DecimalTextInput aria-label="Estimate Price" displayDecimals={2} value={customSellPrice ?? pricing.calculatedSellPrice} onValueChange={setCustomSellPrice} className="mt-2 h-11 w-full rounded-md border border-brand-100 bg-white px-3 text-right text-base font-semibold text-brand-900 focus:outline-none focus:ring-2 focus:ring-accent-500/40 dark:border-brand-600 dark:bg-brand-700 dark:text-brand-50" /></label>}
