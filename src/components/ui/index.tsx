...
export function Modal({ open, onClose, title, children, footer, wide = false, size = 'default' }: ModalProps) {
  if (!open) return null;
  const maxWidth = size === 'large' ? 'max-w-5xl' : wide || size === 'wide' ? 'max-w-4xl' : 'max-w-lg';
  return (
