...
export default function JobAnalysisSummary({ performance }: { performance: JobPerformance }) {
  ...
  const chartSegments = useMemo(() => (mode === 'estimated' ? deferredPerformance.economics.estimatedChartSegments : deferredPerformance.economics.actualChartSegments).filter((segment) => segment.amount > 0), [deferredPerformance, mode]);
  ...
  return <Card className="overflow-hidden">
    <div className="grid lg:grid-cols-[minmax(0,0.9fr)_minmax(360px,1.1fr)]">
      ...
      <section className="border-t border-gray-200 bg-gray-50 p-5 sm:p-6 lg:border-l lg:border-t-0 dark:border-brand-600 dark:bg-brand-800/40" aria-labelledby="cost-distribution-heading">
        ...
        {mode !== 'estimated' && !deferredPerformance.economics.actualCostComplete ? <div className="mt-4 flex gap-2 border-t border-amber-200 pt-4 text-sm text-amber-800 dark:border-amber-800 dark:text-amber-200"><AlertCircle className="mt-0.5 shrink-0" size={16} /><p>Incomplete cost data: {deferredPerformance.costs.unavailableCategories.join(', ')} unavailable.</p></div> : null}
