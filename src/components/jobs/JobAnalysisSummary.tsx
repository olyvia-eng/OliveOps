  ...
  return <Card className="overflow-hidden">
    <div className="grid lg:grid-cols-[minmax(0,0.9fr)_minmax(360px,1.1fr)]">
      <section className="p-5 sm:p-6" aria-labelledby="job-financial-summary-heading">
        ...
        {mode !== 'estimated' ? <div className="mt-4 border-t border-gray-200 pt-4 text-xs text-gray-500 dark:border-brand-600 dark:text-brand-300"><p>{mode === 'actual' ? 'This is the contract value less actual costs recorded to date. It is not the final Job profit until all costs are recorded.' : deferredPerformance.costs.varianceConvention}</p>{!deferredPerformance.economics.actualCostComplete ? <p className="mt-2 font-medium text-amber-700">{deferredPerformance.profit.unavailableReason}</p> : null}</div> : null}
      </section>
      ...
      <section className="border-t border-gray-200 bg-gray-50 p-5 sm:p-6 lg:border-l lg:border-t-0 dark:border-brand-600 dark:bg-brand-800/40" aria-labelledby="cost-distribution-heading">
        ...
        {mode === 'actual' && !deferredPerformance.economics.actualCostComplete ? <div className="mt-4 flex gap-2 border-t border-amber-200 pt-4 text-sm text-amber-800 dark:border-amber-800 dark:text-amber-200"><AlertCircle className="mt-0.5 shrink-0" size={16} /><p>{deferredPerformance.economics.statusMessage}</p></div> : null}
