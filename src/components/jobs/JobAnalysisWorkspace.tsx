...
export default function JobAnalysisWorkspace({ job }: { job: Job }) {
  ...
  return <div className="space-y-5">
    ...
    <Card className="p-4"><h3 className="font-semibold text-gray-900">Time Analysis</h3><div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-4"><Metric label="Estimated hours" value={analysis.labour.estimated.hours.toFixed(1)} /><Metric label="Scheduled hours" value={analysis.labour.scheduled.hoursAvailable ? analysis.labour.scheduled.hours.toFixed(1) : 'Unavailable'} /><Metric label="Actual hours" value={analysis.labour.actual.hours.toFixed(1)} /><Metric label="Actual labour cost" value={money(analysis.labour.actual.cost)} /></div>
      {analysis.labour.estimated.hours > 0 ? <div className="mt-5"><div className="mb-1 flex justify-between text-xs text-gray-500"><span>Estimated hours used</span><span>{Math.round((analysis.labour.actual.hours / analysis.labour.estimated.hours) * 100)}%</span></div><div className="h-2 overflow-hidden rounded bg-gray-100" role="progressbar" aria-label="Estimated hours used" aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.round((analysis.labour.actual.hours / analysis.labour.estimated.hours) * 100)}><div className={`h-full rounded ${analysis.labour.actual.hours > analysis.labour.estimated.hours ? 'bg-red-600' : 'bg-brand-600'}`} style={{ width: `${Math.min((analysis.labour.actual.hours / analysis.labour.estimated.hours) * 100, 100)}%` }} /></div></div> : null}
