...
const categoryTerms = (category: BudgetDivisionPlanningItem["category"]) => {
  if (category === "labour")
    return {
      label: "Labour",
      pool: "Labour Overhead Pool",
      denominator: "Billable Labour Hours",
      missing: "billable labour hours",
    };
  if (category === "equipment")
    return {
      label: "Equipment",
      pool: "Equipment Recovery Pool",
      denominator: "Annual Equipment Cost",
      missing: "annual equipment cost",
    };
  if (category === "materials")
    return {
      label: "Material",
      pool: "Material Recovery Pool",
      denominator: "Annual Material Cost",
      missing: "annual material cost",
    };
  return {
    label: "Subcontractor",
    pool: "Subcontractor Recovery Pool",
    denominator: "Annual Subcontractor Cost",
    missing: "annual subcontractor cost",
  };
};

const unavailableCostReason = (
  category: BudgetDivisionPlanningItem["category"],
) => {
  if (category === "labour") return "Complete labour cost information first.";
  if (category === "equipment")
    return "Complete equipment cost and sellable-hours information first.";
  if (category === "materials")
    return "Complete material cost information first.";
...
export default function BudgetPricingAnalysis({
  ...
  const divisions = useMemo(
    () =>
      budgetDivisions.filter(
        (division) =>
          division.budgetId === budget.id && division.status === "active",
      ),
    [budget.id, budgetDivisions],
  );
  const resolvedPlanningItems = useMemo(
    () => prepareBudgetPricingInputs({ planningItems, employees }),
    [employees, planningItems],
  );
  const rows = useMemo(
    () =>
      buildBudgetPricingRows({
            ...
            return (
              <tr key={row.key} id={`pricing-${row.divisionId}-${row.item.id}`}>
                ...
                <td className="px-4 py-3 text-right">
                  {row.costRate > 0
                    ? `${formatCurrency(row.costRate)}/${row.unit}`
                    : "Unavailable"}
                </td>
                <td className="px-4 py-3 text-right">
                ...
                <td className="px-4 py-3 text-right">
                  {isUnavailable ? (
                    <>
                      ...
                      <p className="mt-1 text-xs font-normal text-gray-500 dark:text-brand-300">
                        {row.recoveryUnavailableReason === "configuration"
                          ? "Set recovery percentages to total 100%."
                          : row.recoveryUnavailable
                            ? `Overhead cannot be recovered without planned ${categoryTerms(row.item.category).missing}.`
                            : unavailableCostReason(row.item.category)}
                      </p>
                    </>
                  ) : (
                    <details>
    ...
    <div className="space-y-5">
      <section className="space-y-3">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-brand-300">
            Choose how each Division recovers its allocated overhead through
            customer pricing.
          </p>
        </div>
        {divisions.map((division) => (
          <OverheadRecoveryEditor
            ...
            canEdit={canEdit}
            onSave={(overheadRecoveryPolicy) =>
              updateBudgetDivision(budget.id, division.id, {
        ...
        {divisions.length === 0 ? (
          <Card>
            <EmptyState
              ...
              description="Add a Division before configuring overhead recovery."
            />
          </Card>
        ) : null}
      </section>
      ...
      {rows.length === 0 ? (
        ...
        <section className="space-y-3">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500 dark:text-brand-300">
              Review Budget-calculated pricing by Division from cost through
          ...
          {recoveryWarnings.length > 0 ? (
            ...
            >
              <p className="font-semibold">
                Some overhead cannot be recovered from the current plan.
              </p>
          ...
          <div className="overflow-x-auto">
            <div
              className="inline-flex min-w-max rounded-xl border border-brand-100 bg-white p-1 dark:border-brand-600 dark:bg-brand-700"
              role="tablist"
              ...
              {pricingTabs.map((tab) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    role="tab"
                    aria-selected={activePricingTab === tab.key}
                    aria-controls={`${tab.key}-pricing-panel`}
                    onClick={() => setActivePricingTab(tab.key)}
                    className={`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-colors ${activePricingTab === tab.key ? "bg-brand-600 text-white" : "text-gray-600 hover:bg-brand-50 dark:text-brand-200 dark:hover:bg-brand-800"}`}
                  >
                    <Icon size={16} />
                    {tab.label}
                  </button>
          ...
          <Card
            ...
            id={`${activePricingTab}-pricing-panel`}
            role="tabpanel"
          >
