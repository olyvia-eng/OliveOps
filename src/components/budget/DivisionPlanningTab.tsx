...
export default function DivisionPlanningTab({ budget, division, category, canEdit }: Props) {
  ...
  const openNew = () => { const equipmentDefaults = emptyEquipmentInfoFormValue(); setDraft(category === 'labour' ? { compType: 'hourly', plannedHours: 1900, labourClassification: 'billable', expectedBillablePct: 80, overtimeHours: 0, overtimeMultiplier: 1.5, divisionAllocations: activeDivisions.map((item) => ({ divisionId: item.id, percentage: item.id === division.id ? 100 : 0 })) } : category === 'equipment' ? { classification: equipmentDefaults.equipmentClassification, costType: equipmentDefaults.equipmentCostType, equipmentPayment: 0, equipmentPaymentFrequencyPerYear: 12, yearlyFuelCost: 0, yearlyInsuranceCost: 0, yearlyMaintenanceCost: 0, sellableHoursPerYear: 0, equipmentHoursPerDay: 8, equipmentDivisionAllocations: [{ divisionId: division.id, months: 12 }] } : { unit: 'each', plannedQuantity: 1 }); setEquipmentError(''); setShowEquipmentCalcDetails(false); setEditing('new'); };
  const openEdit = (item: BudgetDivisionPlanningItem) => { const savedLabourAllocations = item.divisionAllocations?.length ? item.divisionAllocations : [{ divisionId: item.divisionId, percentage: 100 }]; setDraft({ ...item, labourClassification: item.labourClassification ?? 'billable', expectedBillablePct: item.expectedBillablePct ?? 0, overtimeHours: item.overtimeHours ?? 0, overtimeMultiplier: item.overtimeMultiplier ?? 1.5, divisionAllocations: activeDivisions.map((value) => ({ divisionId: value.id, percentage: savedLabourAllocations.find((allocation) => allocation.divisionId === value.id)?.percentage ?? 0 })), equipmentDivisionAllocations: item.equipmentDivisionAllocations?.length ? item.equipmentDivisionAllocations : [{ divisionId: item.divisionId, months: item.allocationMonths ?? 12 }] }); setEquipmentError(''); setShowEquipmentCalcDetails(false); setEditing(item); };
  ...
  const splitLabourEvenly = () => setDraft((current) => ({ ...current, divisionAllocations: splitLabourAllocationsEvenly(activeDivisions.map((item) => item.id)) }));
  ...
  const setEquipmentFormValue = (value: EquipmentInfoFormValue) => setDraft((current) => ({
    ...current,
  ...
  const setEquipmentDivisionAllocation = (divisionId: string, months: number) => setDraft((current) => ({
    ...current,
  ...
  const save = async () => {
    ...
    if (category === 'equipment') {
      ...
      if (!equipmentId) {
        const created = await addEquipmentAsset({ name: normalized.description, type: normalized.costCode || 'General Equipment', status: 'available', costType: normalized.equipmentCostType, equipmentClassification: normalized.equipmentClassification, serialNumber: '', hourlyCost: equipmentCostBreakdown.totalCostPerHour, equipmentPayment: normalized.equipmentPayment, equipmentPaymentFrequencyPerYear: normalized.equipmentPaymentFrequencyPerYear, yearlyFuelCost: normalized.yearlyFuelCost, yearlyInsuranceCost: normalized.yearlyInsuranceCost, yearlyMaintenanceCost: normalized.yearlyMaintenanceCost, notes: '' });
      ...
      nextDraft = { ...nextDraft, equipmentId, name: normalized.description, description: normalized.description, costCode: normalized.costCode, costType: normalized.equipmentCostType, classification: normalized.equipmentClassification, equipmentPayment: normalized.equipmentPayment, equipmentPaymentFrequencyPerYear: normalized.equipmentPaymentFrequencyPerYear, paymentFrequencyPerYear: undefined, yearlyFuelCost: normalized.yearlyFuelCost, yearlyInsuranceCost: normalized.yearlyInsuranceCost, yearlyMaintenanceCost: normalized.yearlyMaintenanceCost, sellableHoursPerYear: normalized.sellableHoursPerYear, utilizationHours: undefined, equipmentHoursPerDay: normalized.equipmentHoursPerDay, allocationMonths: undefined, plannedAmount: equipmentCostBreakdown.totalEquipmentCostPerYear };
    }
    const result = editing === 'new'
      ? await addBudgetDivisionPlanningItem({ ...nextDraft, budgetId: budget.id, divisionId: division.id, category } as Omit<BudgetDivisionPlanningItem, 'id' | 'sortOrder' | 'createdAt' | 'updatedAt'>)
      : editing ? await updateBudgetDivisionPlanningItem(editing, nextDraft) : null;
    setSaving(false);
  ...
  const equipmentMonthsForDivision = (item: BudgetDivisionPlanningItem) => item.equipmentDivisionAllocations?.find((allocation) => allocation.divisionId === division.id)?.months
    ?? (item.divisionId === division.id ? item.allocationMonths ?? 12 : 0);
  ...
  return <div className="space-y-4">
    {items.length === 0 ? <Card><EmptyState icon={<Icon />} title={settings.title} description={settings.description} action={actions} /></Card> : <>
      ...
      <Card className="overflow-hidden rounded-lg"><div className="overflow-x-auto"><table className="w-full min-w-[680px] text-sm"><thead className="border-b border-brand-100 bg-brand-50 text-left text-xs uppercase text-brand-400 dark:border-brand-600 dark:bg-brand-800"><tr>{category !== 'equipment' && category !== 'labour' ? <th className="w-24 px-3 py-3">Order</th> : null}<th className="px-3 py-3">{settings.singular}</th><th className="px-3 py-3">Planning assumptions</th><th className="px-3 py-3 text-right">Annual Cost</th>{canEdit ? <th className="w-24 px-3 py-3 text-right">Actions</th> : null}</tr></thead><tbody className="divide-y divide-brand-100 dark:divide-brand-600">{items.map((item, index) => { const labour = calculateDivisionLabour(item); const labourShare = calculateDivisionLabourShare(item, division.id); return <tr key={item.id} draggable={canEdit && category !== 'equipment' && category !== 'labour'} onDragStart={() => setDraggedId(item.id)} onDragOver={(event) => event.preventDefault()} onDrop={() => { if (draggedId) reorder(draggedId, item.id); setDraggedId(null); }} className="text-brand-800 dark:text-brand-100">{category !== 'equipment' && category !== 'labour' ? <td className="px-3 py-3"><div className="flex items-center gap-1 text-brand-400"><GripVertical size={16} /><button type="button" aria-label={`Move ${item.name ?? item.description} earlier`} disabled={!canEdit || index === 0} onClick={() => move(item, -1)}><ArrowUp size={15} /></button><button type="button" aria-label={`Move ${item.name ?? item.description} later`} disabled={!canEdit || index === items.length - 1} onClick={() => move(item, 1)}><ArrowDown size={15} /></button></div></td> : null}<td className="px-3 py-3"><p className="font-semibold">{item.name || item.description}</p>{item.description && item.name !== item.description ? <p className="text-xs text-brand-400">{item.description}</p> : null}</td><td className="px-3 py-3 text-brand-500 dark:text-brand-300">{category === 'labour' ? <><p>{item.role ?? item.compType ?? 'Labour'} · {labour.classification === 'overhead' ? 'Overhead' : `${labour.expectedBillablePct}% billable · ${labourShare.expectedBillableHours.toFixed(0)} allocated hrs · ${formatCurrency(labour.directCostPerBillableHour)}/hr`}</p><p className="mt-1 text-xs font-medium text-brand-500">Allocation: {labourShare.percentage}% to {division.name}</p></> : category === 'equipment' ? `${item.classification ?? 'billable'} · ${equipmentMonthsForDivision(item)} months · ${formatCurrency(item.yearlyFuelCost ?? 0)} yearly fuel` : `${item.plannedQuantity ?? 1} ${item.unit ?? 'each'} × ${formatCurrency(item.unitCost ?? item.rate ?? 0)}`}</td><td className="px-3 py-3 text-right font-semibold">{formatCurrency(plannedAmount(item))}</td>{canEdit ? <td className="px-3 py-3"><div className="flex justify-end gap-1"><button type="button" title="Edit" onClick={() => openEdit(item)} className="grid h-8 w-8 place-items-center rounded-md text-brand-500 hover:bg-brand-50"><Pencil size={15} /></button><button type="button" title="Remove" onClick={() => void deleteBudgetDivisionPlanningItem(item)} className="grid h-8 w-8 place-items-center rounded-md text-accent-700 hover:bg-accent-50"><Trash2 size={15} /></button></div></td> : null}</tr>; })}</tbody></table></div></Card>
    </>}

    <Modal open={editing !== null} onClose={() => { if (!saving) setEditing(null); }} title={`${editing === 'new' ? 'Add' : 'Edit'} ${settings.singular}`} size={category === 'equipment' ? 'large' : 'wide'} footer={<><Button variant="secondary" onClick={() => setEditing(null)} disabled={saving}>Cancel</Button><Button onClick={() => void save()} disabled={saving || (!draft.name && !draft.description && category !== 'equipment') || !labourAllocationValid || !labourInputsValid || !equipmentAllocationValid}>{saving ? 'Saving...' : category === 'equipment' ? (editing === 'new' && draft.equipmentId ? 'Add to Budget' : editing === 'new' ? 'Save Equipment' : 'Save Changes') : `Save ${settings.singular}`}</Button></>}>
      <div className="grid gap-4 sm:grid-cols-2">
        {category === 'labour' ? <>
          ...
          <Select label="Employee Catalog" value={draft.employeeId ?? ''} disabled={editing !== 'new' && Boolean(draft.employeeId)} onChange={(event) => { const employee = employees.find((item) => item.id === event.target.value); setDraft((current) => ({ ...current, employeeId: event.target.value || undefined, name: employee?.name ?? current.name, role: employee?.role ?? current.role, hourlyRate: employee?.hourlyRate ?? current.hourlyRate })); }}><option value="">Custom or unfilled role</option>{employees.filter((item) => item.active).map((item) => { const existing = budgetLabourItems.find((value) => value.employeeId === item.id); return <option key={item.id} value={item.id} disabled={Boolean(existing)}>{item.name}{existing ? ` — Already in Budget (${labourAllocationForDivision(existing, division.id)}% allocated to ${division.name})` : ''}</option>; })}</Select>
          {editing === 'new' && budgetLabourItems.some((item) => item.employeeId) ? <div className="sm:col-span-2 rounded-lg border border-brand-100 bg-brand-50 p-3 dark:border-brand-600 dark:bg-brand-800"><p className="text-xs font-semibold uppercase text-brand-400">Already in Budget</p><div className="mt-2 flex flex-wrap gap-2">{budgetLabourItems.filter((item) => item.employeeId).map((item) => <Button key={item.id} type="button" variant="secondary" size="sm" onClick={() => openEdit(item)}>{item.name ?? item.description} · {labourAllocationForDivision(item, division.id)}% allocated to {division.name} · Edit Allocation</Button>)}</div></div> : null}
          ...
          <section className="sm:col-span-2"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-semibold text-brand-900 dark:text-brand-50">Allocate Employee Cost Across Divisions</h3><p className="mt-1 text-xs text-brand-400">What proportion of this employee's annual labour cost belongs to each Division.</p></div><Button type="button" variant="secondary" size="sm" onClick={splitLabourEvenly}>Split Evenly</Button></div><div className="mt-3 space-y-2">{activeDivisions.map((item) => { const percentage = draft.divisionAllocations?.find((allocation) => allocation.divisionId === item.id)?.percentage ?? 0; return <div key={item.id} className="grid grid-cols-[minmax(0,1fr)_7rem_auto] items-center gap-2"><label htmlFor={`labour-allocation-${item.id}`} className="text-sm">{item.name}{item.id === division.id ? <span className="ml-2 text-xs text-brand-500">Current Division</span> : null}</label><Input id={`labour-allocation-${item.id}`} type="number" min={0} max={100} step={0.01} value={percentage} onChange={(event) => setDivisionAllocation(item.id, numberValue(event.target.value))} /><span className="text-sm text-brand-400">%</span></div>; })}</div><div className={`mt-3 text-sm font-semibold ${labourAllocationValid ? 'text-green-700' : allocationTotal > 100 ? 'text-accent-700' : 'text-amber-700'}`}>{labourAllocationValid ? 'Allocated: 100% · Remaining: 0%' : allocationTotal > 100 ? `Allocated: ${allocationTotal}% · ${allocationTotal - 100}% over allocation` : `Allocated: ${allocationTotal}% · Remaining: ${100 - allocationTotal}%`}</div></section>
        </> : null}
        {category === 'equipment' ? <div className="space-y-6 sm:col-span-2">
          <section>
            ...
            <div className="mt-3"><Select label="Existing Equipment (optional)" value={draft.equipmentId ?? ''} onChange={(event) => { const asset = equipmentAssets.find((item) => item.id === event.target.value); setEquipmentError(''); setDraft((current) => asset ? ({ ...current, equipmentId: asset.id, name: asset.name, description: asset.name, costCode: asset.type, costType: asset.costType, classification: asset.equipmentClassification ?? 'billable', equipmentPayment: asset.equipmentPayment ?? 0, equipmentPaymentFrequencyPerYear: asset.equipmentPaymentFrequencyPerYear ?? 12, paymentFrequencyPerYear: undefined, yearlyFuelCost: asset.yearlyFuelCost ?? 0, yearlyInsuranceCost: asset.yearlyInsuranceCost ?? 0, yearlyMaintenanceCost: asset.yearlyMaintenanceCost ?? 0 }) : ({ ...current, ...emptyEquipmentInfoFormValue(), equipmentId: undefined, name: '', description: '', costCode: '' })); }}><option value="">Create new equipment</option>{equipmentAssets.map((item) => <option key={item.id} value={item.id} disabled={budgetDivisionPlanningItems.some((value) => value.budgetId === budget.id && value.category === 'equipment' && value.id !== (editing === 'new' ? '' : editing?.id) && value.equipmentId === item.id)}>{item.name}</option>)}</Select></div>
          </section>
          ...
          <section className="border-t border-gray-200 pt-5">
            ...
            <div className="mt-3 space-y-2">{activeDivisions.map((item) => { const months = draft.equipmentDivisionAllocations?.find((allocation) => allocation.divisionId === item.id)?.months ?? 0; return <div key={item.id} className="grid items-center gap-3 rounded-lg border border-gray-200 bg-gray-50 p-3 sm:grid-cols-[minmax(0,1fr)_9rem_10rem]"><label htmlFor={`equipment-allocation-${item.id}`} className="text-sm font-medium text-gray-900">{item.name}{item.id === division.id ? <span className="ml-2 text-xs font-normal text-brand-600">Current Division</span> : null}</label><div className="flex items-center gap-2"><Input id={`equipment-allocation-${item.id}`} type="number" min={0} max={12} step={0.25} value={months} onChange={(event) => setEquipmentDivisionAllocation(item.id, numberValue(event.target.value))} /><span className="text-xs text-gray-500">months</span></div><p className="text-right text-sm font-semibold text-gray-900">{formatCurrency(equipmentCostBreakdown.totalEquipmentCostPerYear * months / 12)}</p></div>; })}</div>
            <p className={`mt-3 text-sm font-semibold ${equipmentAllocationValid ? 'text-green-700' : equipmentAllocationTotal > 12 ? 'text-accent-700' : 'text-amber-700'}`}>{equipmentAllocationTotal} of 12 months allocated{equipmentAllocationTotal < 12 ? ` · ${12 - equipmentAllocationTotal} remaining` : equipmentAllocationTotal > 12 ? ` · ${equipmentAllocationTotal - 12} over allocation` : ''}</p>
          </section>
