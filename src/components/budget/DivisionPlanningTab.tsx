...
export default function DivisionPlanningTab({ budget, division, category, canEdit }: Props) {
  ...
  const openEdit = (item: BudgetDivisionPlanningItem) => { setDraft({ ...item, labourClassification: item.labourClassification ?? 'billable', expectedBillablePct: item.expectedBillablePct ?? 0, overtimeHours: item.overtimeHours ?? 0, overtimeMultiplier: item.overtimeMultiplier ?? 1.5, divisionAllocations: item.divisionAllocations?.length ? item.divisionAllocations : [{ divisionId: item.divisionId, percentage: 100 }], equipmentDivisionAllocations: item.equipmentDivisionAllocations?.length ? item.equipmentDivisionAllocations : [{ divisionId: item.divisionId, months: item.allocationMonths ?? 12 }] }); setEquipmentError(''); setShowEquipmentCalcDetails(false); setEditing(item); };
