...
import { getJobSop } from '../jobs/jobSopApi';
...
export default function SopDetailPage({ jobContext = false }: { jobContext?: boolean }) {
  ...
  const load = useCallback(async () => {
    try {
      if (jobContext) {
        const payload = await getJobSop(jobId, sopId);
        const sop = payload.sop;
        setDefinition({ ...sop, id: sop.sopId, status: 'published', active: true, currentVersion: sop.version, createdAt: sop.publishedAt, updatedAt: sop.publishedAt });
        setVersions([]);
        setError('');
        return;
      }
