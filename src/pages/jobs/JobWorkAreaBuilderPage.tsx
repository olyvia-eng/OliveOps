...
import { Badge, Button, Card, DecimalTextInput, EmptyState, Input, PageHeader, Select } from '../../components/ui';
        ...
        {lines.map((line) => {
          ...
          return <div key={line.id} className="border-b border-brand-100 bg-brand-50/40 last:border-b-0 dark:border-brand-600 dark:bg-brand-900/20">
            <div className={`grid min-w-[1060px] ${grid} items-center gap-3 px-3 py-3 text-sm`}>
              ...
              {labour ? <><input aria-label={`Workers for ${line.itemName}`} type="text" inputMode="numeric" value={workers} onChange={(event) => { const nextWorkers = Math.max(1, Math.floor(parseNumericInputValue(event.target.value) || 1)); updateLine(line.id, { workers: nextWorkers, quantity: nextWorkers * hoursPerWorker }); }} onFocus={(event) => event.currentTarget.select()} className="h-9 w-16 rounded-md border border-brand-100 bg-white px-2 text-right text-sm font-semibold dark:border-brand-600 dark:bg-brand-700" /><label className="flex items-center gap-1.5"><DecimalTextInput aria-label={`Hours per worker for ${line.itemName}`} value={hoursPerWorker} onValueChange={(hours) => { const nextHours = Math.max(0, hours); updateLine(line.id, { hoursPerWorker: nextHours, quantity: workers * nextHours }); }} className="h-9 w-20 rounded-md border border-brand-100 bg-white px-2 text-right text-sm font-semibold dark:border-brand-600 dark:bg-brand-700" /><span className="text-xs text-gray-500">hr</span></label></> : <label className="flex items-center gap-1.5"><DecimalTextInput aria-label={`${line.unit === 'hr' ? 'Hours' : 'Quantity'} for ${line.itemName}`} value={line.quantity} onValueChange={(value) => updateLine(line.id, { quantity: Math.max(0, value) })} className="h-9 w-20 rounded-md border border-brand-100 bg-white px-2 text-right text-sm font-semibold dark:border-brand-600 dark:bg-brand-700" /><span className="text-xs text-gray-500">{line.unit}</span></label>}
