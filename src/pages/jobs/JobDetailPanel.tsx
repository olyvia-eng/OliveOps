    ...
    <div className="min-w-0">
      ...
      <div className="space-y-4 p-4 sm:p-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          {canViewFinancials ? <Card className="p-3"><p className="text-xs text-gray-500 dark:text-brand-200">Actual Cost to Date</p><p className="mt-1 text-lg font-semibold text-gray-900 dark:text-brand-50">{performance?.costs.knownActualIncludingOverhead === null || !performance ? 'Unavailable' : formatCurrency(performance.costs.knownActualIncludingOverhead)}</p><p className="text-xs text-gray-400">{performance?.costs.actualComplete ? 'Recorded cost to date' : 'Actual cost unavailable'}</p></Card> : null}
        </div>
