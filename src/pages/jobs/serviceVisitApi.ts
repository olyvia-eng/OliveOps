...
export const updateServiceVisitAssignments = (visit: ServiceVisit, assignedEmployeeIds: string[], assignedEquipmentIds: string[], applyToFuture = false) => request<{ ok: true; visit: ServiceVisit; updatedVisits: ServiceVisit[]; appliedToFutureCount: number }>({ method: 'PATCH', action: 'assignments', jobId: visit.jobId, visitId: visit.id, body: { serviceId: visit.serviceId, visitId: visit.id, revision: visit.revision, assignedEmployeeIds, assignedEquipmentIds, applyToFuture } });
