...
const emptyLine = (): InvoiceLineItem => ({ id: generateId(), category: 'contract_service', description: '', quantity: 1, unit: 'job', unitPrice: 0, unitPriceBeforeTax: 0, amount: 0, taxable: true });
...
const invoiceTypeDescriptions: Record<InvoiceType, string> = { deposit: 'Collect an upfront payment', progress: 'Bill part of the contract', final: 'Bill the remaining balance', custom: 'Build an invoice manually' };
...
const draftFingerprint = (value: Pick<Form, 'jobId' | 'invoiceType' | 'issueDate' | 'dueDate' | 'paymentTermsDays' | 'taxRate' | 'notes' | 'lineItems' | 'overContractConfirmed'>) => JSON.stringify({
  jobId: value.jobId, invoiceType: value.invoiceType, issueDate: value.issueDate, dueDate: value.dueDate,
  paymentTermsDays: value.paymentTermsDays, taxRate: value.taxRate, notes: value.notes.trim(),
  overContractConfirmed: value.overContractConfirmed,
  lineItems: value.lineItems.map((line) => ({ id: line.id, category: line.category, description: line.description, quantity: line.quantity, unit: line.unit, unitPriceBeforeTax: line.unitPriceBeforeTax, taxable: line.taxable, sourceWorkAreaId: line.sourceWorkAreaId, sourceLineItemId: line.sourceLineItemId })),
});
...
export default function InvoicesPage() {
  ...
  const draftDirty = Boolean(selected?.status === 'draft' && draftFingerprint(form) !== draftFingerprint({ jobId: selected.jobId, invoiceType: selected.invoiceType ?? 'custom', issueDate: selected.issueDate, dueDate: selected.dueDate, paymentTermsDays: selected.paymentTermsDays ?? 30, taxRate: selected.taxRate ?? 0, notes: selected.notes, lineItems: selected.lineItems ?? [], overContractConfirmed: Boolean(selected.overContract) }));
  const saveDisabledReason = !job
    ? 'Select a Job to save this draft.'
    : lineValidationError || summary.subtotal <= 0 || summary.amount <= 0
      ? 'Add a billable amount to save this draft.'
      : requiredFieldError
        ? 'Complete the required invoice details to save this draft.'
        : exceeds && form.invoiceType !== 'custom'
          ? 'This invoice exceeds the remaining contract amount.'
          : exceeds && !form.overContractConfirmed
            ? 'Confirm intentional over-contract billing to save this draft.'
            : '';
  ...
  const applyAmount = (next: Form, nextPosition = position) => {
    ...
    return { ...next, lineItems: [{ ...emptyLine(), description, taxable, unitPrice: amount, unitPriceBeforeTax: amount }] };
  };
  const start = () => { setSelected(null); setForm(emptyForm()); setError(''); setOpen(true); };
  const view = (invoice: Invoice) => { const invoiceType = invoice.invoiceType ?? 'custom'; const hasSourceLines = invoice.lineItems?.some((line) => line.sourceLineItemId) ?? false; setSelected(invoice); setError(''); setVoidReason(''); setForm({ jobId: invoice.jobId, invoiceType, issueDate: invoice.issueDate, dueDate: invoice.dueDate, paymentTermsDays: invoice.paymentTermsDays ?? 30, taxRate: invoice.taxRate ?? 0, notes: invoice.notes, lineItems: invoice.lineItems?.map((line) => ({ ...line })) ?? [], amountMode: invoiceType === 'progress' && hasSourceLines ? 'work_areas' : 'fixed', billingAmount: invoice.subtotal ?? invoice.amount, billingPercent: 0, overContractConfirmed: Boolean(invoice.overContract) }); setOpen(true); };
  ...
  const chooseType = (invoiceType: InvoiceType) => {
    ...
    const reset = { ...form, invoiceType, amountMode: 'fixed' as const, billingAmount: 0, billingPercent: 0, lineItems: [] };
    if (invoiceType === 'custom') return setForm({ ...reset, lineItems: [emptyLine()] });
    setForm(applyAmount(reset));
  };
  const updateLine = <K extends keyof InvoiceLineItem>(id: ID, key: K, value: InvoiceLineItem[K]) => setForm((current) => {
    ...
    return { ...current, lineItems: current.lineItems.map((line) => line.id === id ? { ...line, [key]: value } : line) };
  });
  ...
  return <div>
    ...
    {open ? <div className="fixed inset-0 z-50"><button aria-label="Close invoice drawer" className="absolute inset-0 bg-black/40" onClick={() => !saving && setOpen(false)} /><aside className="absolute inset-y-0 right-0 flex w-full max-w-7xl flex-col border-l border-brand-100 bg-brand-50 shadow-2xl dark:border-brand-600 dark:bg-brand-800"><header className="flex h-16 items-center justify-between border-b border-brand-100 bg-white px-5 dark:border-brand-600 dark:bg-brand-700"><div><p className="text-xs font-semibold uppercase text-brand-400">{!selected ? 'New draft invoice' : selected.status === 'draft' ? 'Draft invoice' : 'Invoice'}</p><div className="flex items-center gap-2"><h2 className="text-lg font-semibold">{selected?.number ?? 'Invoice number assigned when saved'}</h2>{selected ? <Badge label={displayStatus(selected)} className={badge[displayStatus(selected)]} /> : null}</div></div><button aria-label="Close" className="h-10 w-10 rounded-lg hover:bg-brand-100" onClick={() => setOpen(false)}><X className="mx-auto" /></button></header><div className="flex-1 overflow-y-auto"><div className="grid min-h-full xl:grid-cols-[minmax(0,1fr)_340px]"><main className="space-y-6 p-5 lg:p-7">
        ...
        <section><h3 className="mb-3 text-sm font-semibold">2. Invoice type and billing method</h3><div className="grid grid-cols-2 gap-2 lg:grid-cols-4">{(['deposit', 'progress', 'final', 'custom'] as InvoiceType[]).map((type) => <button disabled={!editable || !job} key={type} onClick={() => chooseType(type)} className={`rounded-lg border px-3 py-2 text-left ${form.invoiceType === type ? 'border-accent-500 bg-accent-50 ring-1 ring-accent-500/20 dark:bg-accent-950/20' : 'border-brand-100 bg-white dark:bg-brand-700'}`}><span className="block text-sm font-semibold capitalize">{type}</span><span className="mt-0.5 block text-xs text-brand-500 dark:text-brand-200">{invoiceTypeDescriptions[type]}</span></button>)}</div>{job && editable && !['custom', 'final'].includes(form.invoiceType) ? <div className="mt-3 grid gap-3 rounded-lg border border-brand-100 bg-white p-3 dark:bg-brand-700 md:grid-cols-3"><Select label="Billing method" value={form.amountMode} onChange={(event) => { const nextMode = event.target.value as Form['amountMode']; const next = { ...form, amountMode: nextMode, lineItems: nextMode === 'work_areas' || form.amountMode === 'work_areas' ? [] : form.lineItems }; setForm(applyAmount(next)); }}><option value="fixed">Fixed pre-tax amount</option><option value="percent">Percentage</option>{form.invoiceType === 'progress' ? <option value="work_areas">Select work-area lines</option> : null}</Select>{form.amountMode === 'fixed' ? <Input label="Pre-tax amount" type="number" min={0.01} value={form.billingAmount} onChange={(event) => { const next = { ...form, billingAmount: Number(event.target.value) }; setForm(applyAmount(next)); }} /> : form.amountMode === 'percent' ? <div><Input label="Percentage of contract" type="number" min={0.01} max={100} value={form.billingPercent} onChange={(event) => { const next = { ...form, billingPercent: Number(event.target.value) }; setForm(applyAmount(next)); }} /><p className="mt-1 text-xs font-medium text-brand-500">Calculated pre-tax amount: {formatCurrency(summary.subtotal)}</p></div> : <p className="self-end pb-2 text-xs text-brand-400 md:col-span-2">Full selected lines will be added to this invoice.</p>}</div> : null}{job && editable && form.invoiceType === 'final' ? <p className="mt-3 text-sm font-medium text-brand-600 dark:text-brand-100">Remaining pre-tax balance: {formatCurrency(position.remainingAmount)}</p> : null}{job && editable && form.invoiceType === 'progress' && form.amountMode === 'work_areas' ? <div className="mt-3 space-y-2">{(job.originalEstimateSnapshot?.workAreas ?? job.operationalWorkAreas ?? []).map((area) => <div key={area.id} className="rounded-lg border bg-white dark:bg-brand-700"><p className="border-b px-4 py-2 text-sm font-semibold">{area.name}</p>{area.lineItems.map((line) => <label key={line.id} className="flex cursor-pointer items-center gap-3 px-4 py-2 text-sm"><input type="checkbox" checked={form.lineItems.some((item) => item.sourceLineItemId === line.id)} onChange={() => toggleSource(line, area.id)} /><span className="flex-1">{line.description || line.itemName}</span><span>{line.quantity} {line.unit}</span><strong>{formatCurrency(linePrice(line) * line.quantity)}</strong></label>)}</div>)}</div> : null}</section>
        <section><div className="mb-3 flex items-end justify-between gap-3"><div><h3 className="text-sm font-semibold">3. Invoice lines</h3>{form.invoiceType !== 'custom' && form.amountMode !== 'work_areas' ? <p className="mt-1 text-xs text-brand-400">Calculated from billing method. Description remains editable while Draft.</p> : null}</div>{editable && form.invoiceType === 'custom' ? <Button size="sm" variant="secondary" onClick={() => setForm((current) => ({ ...current, lineItems: [...current.lineItems, emptyLine()] }))}><Plus /> Add line</Button> : null}</div><div className="space-y-3">{form.lineItems.map((line, index) => { const generatedLumpSum = form.invoiceType !== 'custom' && form.amountMode !== 'work_areas' && !line.sourceLineItemId; return <InvoiceLine key={line.id} line={line} index={index} taxRate={form.taxRate} legacy={Boolean(selected && selected.schemaVersion !== 2)} editable={editable && !line.sourceLineItemId} financialEditable={editable && form.invoiceType === 'custom' && !line.sourceLineItemId} generatedLumpSum={generatedLumpSum} onChange={updateLine} onRemove={() => setForm((current) => ({ ...current, lineItems: current.lineItems.filter((item) => item.id !== line.id) }))} />; })}{!form.lineItems.length ? <p className="rounded-lg border border-dashed p-6 text-center text-sm text-brand-400">Choose a billing amount or select work-area lines.</p> : null}</div></section>
        <section><h3 className="mb-3 text-sm font-semibold">4. Invoice details</h3><div className="grid gap-3 md:grid-cols-4"><Input label="Issue date" type="date" disabled={!editable} value={form.issueDate} onChange={(event) => setForm((current) => ({ ...current, issueDate: event.target.value, dueDate: plusDays(event.target.value, current.paymentTermsDays) }))} /><Input label="Payment terms (days)" type="number" disabled={!editable} value={form.paymentTermsDays} onChange={(event) => { const days = Number(event.target.value); setForm((current) => ({ ...current, paymentTermsDays: days, dueDate: plusDays(current.issueDate, days) })); }} /><Input label="Due date" type="date" disabled={!editable} value={form.dueDate} onChange={(event) => setForm((current) => ({ ...current, dueDate: event.target.value }))} /><Input label="HST rate (%)" type="number" disabled={!editable} value={form.taxRate} onChange={(event) => setForm((current) => ({ ...current, taxRate: Number(event.target.value) }))} /></div></section>
        <section><h3 className="mb-3 text-sm font-semibold">5. Notes</h3><TextArea label="Notes" rows={3} className="resize-y" disabled={!editable} value={form.notes} onChange={(event) => setForm((current) => ({ ...current, notes: event.target.value }))} /></section>
      </main><aside className="border-t bg-white p-5 dark:bg-brand-700 xl:border-l xl:border-t-0"><div className="sticky top-5"><h3 className="text-sm font-semibold">Financial summary</h3><dl className="mt-4 space-y-3 text-sm"><Money label="Contract total" value={selected?.contractAmountSnapshot ?? position.contractAmount} /><Money label="Previously invoiced" value={selected?.previouslyInvoicedSnapshot ?? position.previouslyInvoiced} /><Money label="This invoice subtotal" value={selected ? getInvoiceContractAmount(selected) : summary.subtotal} border /><Money label="HST" value={selected?.taxAmount ?? summary.taxAmount} /><Money label="This invoice total" value={selected?.amount ?? summary.amount} strong /><Money label="Remaining after" value={Math.max(0, position.remainingAmount - summary.subtotal)} border /></dl>{exceeds ? <div className="mt-4 rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-800">Exceeds remaining contract by {formatCurrency(summary.subtotal - position.remainingAmount)}.{form.invoiceType === 'custom' && editable ? <label className="mt-3 flex gap-2 font-medium"><input type="checkbox" checked={form.overContractConfirmed} onChange={(event) => setForm((current) => ({ ...current, overContractConfirmed: event.target.checked }))} /> Confirm intentional over-contract billing</label> : null}</div> : null}{error ? <p className="mt-4 rounded-lg bg-red-50 p-3 text-sm text-red-800">{error}</p> : null}<div className="mt-6 space-y-2">{editable ? <><Button className="w-full" disabled={saving || Boolean(saveDisabledReason) || Boolean(selected && !draftDirty)} title={selected && !draftDirty ? 'No unsaved changes.' : saveDisabledReason || undefined} onClick={() => void save()}>{saving ? 'Saving...' : 'Save draft invoice'}</Button>{saveDisabledReason && !saving ? <p className="text-xs text-brand-400" role="status">{saveDisabledReason}</p> : null}</> : null}{selected?.status === 'draft' && !selected.quickBooksLinked && !qbo[selected.id] ? <><Button className="w-full" variant="secondary" disabled={saving || draftDirty} title={draftDirty ? 'Save draft changes before marking sent.' : undefined} onClick={() => void status(selected, 'sent')}><Send /> Mark sent</Button>{draftDirty ? <p className="text-xs text-brand-400">Save draft changes before marking sent.</p> : null}<Button className="w-full" variant="ghost" onClick={() => void remove(selected)}><Trash2 /> Delete draft</Button></> : null}{selected && !selected.quickBooksLinked && !qbo[selected.id] && ['sent', 'partially_paid', 'overdue'].includes(selected.status) ? <><Input label="Void reason" value={voidReason} onChange={(event) => setVoidReason(event.target.value)} /><Button className="w-full" variant="danger" disabled={saving} onClick={() => void status(selected, 'void')}><Ban /> Void invoice</Button></> : null}{selected && !selected.quickBooksLinked && selected.status !== 'draft' && selected.status !== 'void' && !qbo[selected.id] ? <Button className="w-full" variant="secondary" disabled={!quickBooks.connected} onClick={() => void createInQbo(selected)}><BookOpenCheck /> Create in QuickBooks</Button> : null}</div></div></aside></div></div></aside></div> : null}
  </div>;
}
