...
import BudgetPricingAnalysis from '../../components/budget/BudgetPricingAnalysis';
 ...
 type BudgetTab = 'info' | 'divisions' | 'profit-loss' | 'analysis';
...
export default function BudgetWorkspacePage({ currentUserRole }: Props) {
  ...
  const overheadMigrationStarted = useRef(new Set<string>());
  ...
  useEffect(() => {
    const activeDivisionCount = divisions.filter((item) => item.status === 'active').length;
    if (!budgetId || !canEdit || activeDivisionCount === 0 || overheadMigrationStarted.current.has(budgetId)) return;
    overheadMigrationStarted.current.add(budgetId);
    void migrateLegacyBudgetOverhead(budgetId);
  }, [budgetId, canEdit, divisions, migrateLegacyBudgetOverhead]);
  ...
  const tabs: Array<{ key: BudgetTab; label: string }> = [
    { key: 'info', label: 'Info' }, { key: 'divisions', label: 'Divisions' }, { key: 'profit-loss', label: 'Profit & Loss' }, { key: 'analysis', label: 'Analysis' },
  ];
  ...
  return (
    <div>
      ...
      {activeTab === 'analysis' ? <div className="space-y-5"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5"><Summary label="Revenue" value={formatCurrency(financials.revenue)} /><Summary label="Direct Costs" value={financials.isComplete ? formatCurrency(financials.totalDirectCosts) : '—'} sub={financials.isComplete ? undefined : 'Budget incomplete'} /><Summary label="Gross Profit" value={financials.grossProfit === null ? '—' : formatCurrency(financials.grossProfit)} /><Summary label="Overhead" value={formatCurrency(financials.totalOverhead)} /><Summary label="Net Profit" value={financials.operatingProfit === null ? '—' : formatCurrency(financials.operatingProfit)} /></div><BudgetPricingAnalysis budget={budget} planningItems={budgetPlanningItems} canEdit={canEdit} /><Card className="overflow-hidden"><div className="border-b border-brand-100 px-4 py-3 dark:border-brand-600"><h2 className="font-semibold text-gray-900 dark:text-brand-50">Division Performance</h2></div><div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-brand-50 text-left text-xs uppercase text-gray-500 dark:bg-brand-800"><tr><th className="px-4 py-3">Division</th><th className="px-4 py-3">Revenue Target</th><th className="px-4 py-3">Direct Cost</th><th className="px-4 py-3">Gross Profit</th><th className="px-4 py-3">Margin</th><th className="px-4 py-3">Allocated Overhead</th><th className="px-4 py-3">Operating Profit</th></tr></thead><tbody>{financials.divisions.map((result) => <tr key={result.divisionId} className="border-t border-brand-100 dark:border-brand-600"><td className="px-4 py-3 font-medium">{result.divisionName}</td><td className="px-4 py-3">{formatCurrency(result.revenue)}</td><td className="px-4 py-3">{result.isComplete ? formatCurrency(result.totalDirectCosts) : '—'}</td><td className="px-4 py-3">{result.grossProfit === null ? '—' : formatCurrency(result.grossProfit)}</td><td className="px-4 py-3">{result.grossMargin === null ? '—' : `${result.grossMargin.toFixed(1)}%`}</td><td className="px-4 py-3">{formatCurrency(result.totalOverhead)}</td><td className="px-4 py-3">{result.operatingProfit === null ? '—' : formatCurrency(result.operatingProfit)}</td></tr>)}</tbody></table></div>{activeDivisions.length === 0 ? <EmptyState icon={<BarChart3 />} title="No Division performance yet" description="Add Divisions and revenue targets to begin building this analysis." /> : null}</Card></div> : null}
