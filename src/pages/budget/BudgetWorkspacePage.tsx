...
import { calculateBudgetLabourTotals, calculateDivisionLabourShare } from './divisionLabourPlanningModel';
...
export default function BudgetWorkspacePage({ currentUserRole }: Props) {
  ...
  const { budgets, budgetDivisions, budgetDivisionPlanningItems, updateBudget, addBudgetDivision, updateBudgetDivision } = useStore();
  ...
  const labourTotals = calculateBudgetLabourTotals(budgetLabourItems);
  const divisionLabourDirectCost = (divisionId: string) => budgetLabourItems.reduce((sum, item) => sum + calculateDivisionLabourShare(item, divisionId).directLabourCost, 0);
