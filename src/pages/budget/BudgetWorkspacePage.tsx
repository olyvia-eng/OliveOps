...
import { calculateBudgetFinancials } from './budgetFinancialModel';
import { BudgetProfitLossView } from '../../components/budget/ProfitLossView';
import BudgetPricingAnalysis from '../../components/budget/BudgetPricingAnalysis';

 type BudgetTab = 'info' | 'divisions' | 'company-overhead' | 'profit-loss' | 'analysis';
...
export default function BudgetWorkspacePage({ currentUserRole }: Props) {
  ...
  const { budgets, budgetDivisions, budgetDivisionPlanningItems, budgetItems, updateBudget, addBudgetDivision, updateBudgetDivision } = useStore();
  ...
  const [divisionForm, setDivisionForm] = useState({ name: '', costCode: '', revenueTarget: '', description: '' });
  ...
  const openDivision = (division?: BudgetDivision) => {
    ...
    setDivisionForm({ name: division?.name ?? '', costCode: division?.costCode ?? '', revenueTarget: division ? String(division.revenueTarget) : '', description: division?.description ?? '' });
    setDivisionError('');
  };
  ...
  const saveDivision = async () => {
    ...
    const data = { budgetId: budget.id, name, costCode: divisionForm.costCode.trim(), description: divisionForm.description.trim(), revenueTarget };
  ...
  if (!budget || !form || budget.planningModel !== 'divisions_v1') {
  ...
  const companyOverheadItems = budgetItems.filter((item) => item.budgetId === budget.id && item.category === 'overhead');
  const budgetPlanningItems = budgetDivisionPlanningItems.filter((item) => item.budgetId === budget.id);
  const financials = calculateBudgetFinancials({
    divisions: activeDivisions,
    planningItems: budgetPlanningItems,
    companyOverheadItems,
  });
  ...
  const tabs: Array<{ key: BudgetTab; label: string }> = [
    { key: 'info', label: 'Info' }, { key: 'divisions', label: 'Divisions' }, { key: 'company-overhead', label: 'Company Overhead' }, { key: 'profit-loss', label: 'Profit & Loss' }, { key: 'analysis', label: 'Analysis' },
  ];
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2"><Badge label={form.status} className={statusClass[form.status]} /><span className="text-xs text-gray-500">{form.fiscalYear}</span></div>
      ...
      {activeTab === 'divisions' ? <div className="space-y-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="text-lg font-semibold text-gray-900 dark:text-brand-50">Divisions</h2><p className="mt-1 text-sm text-gray-500 dark:text-brand-300">Manage the operating divisions within this Budget.</p></div>{canEdit ? <Button onClick={() => openDivision()}><Plus size={16} /> Add Division</Button> : null}</div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Summary label="Total Divisions" value={String(activeDivisions.length)} /><Summary label="Total Revenue Target" value={formatCurrency(totalRevenueTarget)} /><Summary label="Total Direct Cost" value={financials.isComplete ? formatCurrency(financials.totalDirectCosts) : '—'} sub={financials.isComplete ? undefined : 'Budget incomplete'} /><Summary label="Budget Gross Margin" value={financials.grossMargin === null ? '—' : `${financials.grossMargin.toFixed(1)}%`} sub={financials.isComplete ? undefined : 'Not calculated yet'} /></div>{divisions.length === 0 ? <Card><EmptyState icon={<Building2 />} title="No divisions yet" description="Add the operating areas you want to plan within this Budget. Examples might include Snow Removal, Landscaping, Excavation, or Maintenance." action={canEdit ? <Button onClick={() => openDivision()}><Plus size={16} /> Add Division</Button> : undefined} /></Card> : <Card className="overflow-hidden"><div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-brand-50 text-left text-xs uppercase text-gray-500 dark:bg-brand-800 dark:text-brand-300"><tr><th className="px-4 py-3">Division</th><th className="px-4 py-3">Cost Code</th><th className="px-4 py-3">Revenue Target</th><th className="px-4 py-3">Direct Cost</th><th className="px-4 py-3">Gross Profit</th><th className="px-4 py-3">Margin</th><th className="px-4 py-3">Status</th><th className="px-4 py-3"><span className="sr-only">Actions</span></th></tr></thead><tbody className="divide-y divide-brand-100 dark:divide-brand-600">{[...activeDivisions, ...archivedDivisions].map((division) => { const result = divisionFinancials.get(division.id); return <tr key={division.id} className="cursor-pointer hover:bg-brand-50 dark:hover:bg-brand-800" onClick={() => navigate(`/budgets/${budget.id}/divisions/${division.id}?tab=profit-loss`)}><td className="px-4 py-3"><p className="font-medium text-gray-900 dark:text-brand-50">{division.name}</p>{division.description ? <p className="mt-1 text-xs text-gray-500">{division.description}</p> : null}</td><td className="px-4 py-3 text-gray-500">{division.costCode || '—'}</td><td className="px-4 py-3">{formatCurrency(division.revenueTarget)}</td><td className="px-4 py-3">{result?.isComplete ? formatCurrency(result.totalDirectCosts) : '—'}</td><td className="px-4 py-3">{result?.grossProfit === null || result?.grossProfit === undefined ? '—' : formatCurrency(result.grossProfit)}</td><td className="px-4 py-3">{result?.grossMargin === null || result?.grossMargin === undefined ? '—' : `${result.grossMargin.toFixed(1)}%`}</td><td className="px-4 py-3"><Badge label={division.status} className={division.status === 'active' ? 'bg-brand-100 text-brand-700' : 'bg-gray-100 text-gray-600'} /></td><td className="px-4 py-3"><div className="flex justify-end gap-1" onClick={(event) => event.stopPropagation()}>{canEdit ? <><Button variant="ghost" size="sm" onClick={() => openDivision(division)} aria-label={`Edit ${division.name}`}><Pencil size={14} /></Button><Button variant="ghost" size="sm" onClick={() => void updateBudgetDivision(budget.id, division.id, { status: division.status === 'active' ? 'archived' : 'active' })} aria-label={`${division.status === 'active' ? 'Archive' : 'Restore'} ${division.name}`}>{division.status === 'active' ? <Archive size={14} /> : <RotateCcw size={14} />}</Button></> : null}</div></td></tr>; })}</tbody></table></div></Card>}</div> : null}

      {activeTab === 'company-overhead' ? companyOverheadItems.length === 0 ? <Card><EmptyState icon={<Wallet />} title="No company overhead yet" description="Company-level overhead entered for this Budget will appear here. It is not silently allocated to Divisions." /></Card> : <div className="space-y-4"><div><h2 className="text-lg font-semibold text-gray-900 dark:text-brand-50">Company Overhead</h2><p className="mt-1 text-sm text-gray-500 dark:text-brand-300">Company-wide costs remain separate from Division Overhead.</p></div><Card className="overflow-hidden"><table className="w-full text-sm"><thead className="bg-brand-50 text-left text-xs uppercase text-gray-500 dark:bg-brand-800 dark:text-brand-300"><tr><th className="px-4 py-3">Expense</th><th className="px-4 py-3">Cost Code</th><th className="px-4 py-3 text-right">Budgeted</th></tr></thead><tbody className="divide-y divide-brand-100 dark:divide-brand-600">{companyOverheadItems.map((item) => <tr key={item.id}><td className="px-4 py-3 font-medium">{item.description}</td><td className="px-4 py-3 text-gray-500">{item.costCode || '—'}</td><td className="px-4 py-3 text-right font-medium">{formatCurrency(item.budgeted)}</td></tr>)}</tbody><tfoot><tr className="border-t-2 border-brand-200 font-semibold dark:border-brand-500"><td className="px-4 py-3 uppercase" colSpan={2}>Total Company Overhead</td><td className="px-4 py-3 text-right">{formatCurrency(financials.companyOverhead)}</td></tr></tfoot></table></Card></div> : null}

      {activeTab === 'profit-loss' ? <BudgetProfitLossView fiscalYear={budget.fiscalYear} financials={financials} onDivisionClick={(divisionId) => navigate(`/budgets/${budget.id}/divisions/${divisionId}?tab=profit-loss`)} /> : null}

      {activeTab === 'analysis' ? <div className="space-y-5"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Summary label="Total Divisions" value={String(activeDivisions.length)} /><Summary label="Revenue Target" value={formatCurrency(financials.revenue)} /><Summary label="Direct Costs" value={financials.isComplete ? formatCurrency(financials.totalDirectCosts) : '—'} sub={financials.isComplete ? undefined : 'Budget incomplete'} /><Summary label="Operating Profit" value={financials.operatingProfit === null ? '—' : formatCurrency(financials.operatingProfit)} /></div><BudgetPricingAnalysis budget={budget} planningItems={budgetPlanningItems} companyOverhead={financials.companyOverhead} canEdit={canEdit} /><Card className="overflow-hidden"><div className="border-b border-brand-100 px-4 py-3 dark:border-brand-600"><h2 className="font-semibold text-gray-900 dark:text-brand-50">Division Performance</h2></div><div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-brand-50 text-left text-xs uppercase text-gray-500 dark:bg-brand-800"><tr><th className="px-4 py-3">Division</th><th className="px-4 py-3">Revenue Target</th><th className="px-4 py-3">Direct Cost</th><th className="px-4 py-3">Gross Profit</th><th className="px-4 py-3">Margin</th><th className="px-4 py-3">Overhead before Company</th><th className="px-4 py-3">Contribution before Company</th></tr></thead><tbody>{financials.divisions.map((result) => <tr key={result.divisionId} className="border-t border-brand-100 dark:border-brand-600"><td className="px-4 py-3 font-medium">{result.divisionName}</td><td className="px-4 py-3">{formatCurrency(result.revenue)}</td><td className="px-4 py-3">{result.isComplete ? formatCurrency(result.totalDirectCosts) : '—'}</td><td className="px-4 py-3">{result.grossProfit === null ? '—' : formatCurrency(result.grossProfit)}</td><td className="px-4 py-3">{result.grossMargin === null ? '—' : `${result.grossMargin.toFixed(1)}%`}</td><td className="px-4 py-3">{formatCurrency(result.totalOverheadBeforeCompany)}</td><td className="px-4 py-3">{result.operatingProfitBeforeCompanyOverhead === null ? '—' : formatCurrency(result.operatingProfitBeforeCompanyOverhead)}</td></tr>)}</tbody></table></div>{activeDivisions.length === 0 ? <EmptyState icon={<BarChart3 />} title="No Division performance yet" description="Add Divisions and revenue targets to begin building this analysis." /> : null}</Card></div> : null}

      <Modal open={divisionModal !== null} onClose={() => { if (!savingDivision) setDivisionModal(null); }} title={divisionModal === 'new' ? 'Add Division' : 'Edit Division'} footer={<><Button variant="secondary" onClick={() => setDivisionModal(null)} disabled={savingDivision}>Cancel</Button><Button onClick={() => void saveDivision()} disabled={savingDivision}>{savingDivision ? 'Saving...' : 'Save Division'}</Button></>}><div className="space-y-4"><Input label="Division Name" required value={divisionForm.name} onChange={(event) => setDivisionForm({ ...divisionForm, name: event.target.value })} placeholder="Snow Removal" /><Input label="Cost Code" value={divisionForm.costCode} onChange={(event) => setDivisionForm({ ...divisionForm, costCode: event.target.value })} placeholder="DIV-SNOW" /><Input label="Revenue Target" inputMode="decimal" value={divisionForm.revenueTarget} onChange={(event) => setDivisionForm({ ...divisionForm, revenueTarget: event.target.value })} placeholder="500000" /><TextArea label="Description" value={divisionForm.description} onChange={(event) => setDivisionForm({ ...divisionForm, description: event.target.value })} />{divisionError ? <p className="text-sm text-red-600">{divisionError}</p> : null}</div></Modal>
    </div>
  );
}
