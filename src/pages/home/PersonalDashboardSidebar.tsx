...
export interface PersonalDashboardSidebarProps {
  selectedDate: Date;
...
export function MiniCalendarWidget({ selectedDate, onSelectDate }: Pick<PersonalDashboardSidebarProps, 'selectedDate' | 'onSelectDate'>) {
  return <Card className="h-full rounded-lg p-4"><MiniMonthCalendar selectedDate={selectedDate} onSelect={onSelectDate} /></Card>;
}

export function UpcomingScheduleWidget({ upcoming, onOpenJob, onOpenTask }: Pick<PersonalDashboardSidebarProps, 'upcoming' | 'onOpenJob' | 'onOpenTask'>) {
  return (
...
export function RecentActivityWidget({ activity }: Pick<PersonalDashboardSidebarProps, 'activity'>) {
  return (
...
export function QuickActionsWidget({ showTimeClock, onAddTask, onOpenSchedule, onOpenTimeClock }: Pick<PersonalDashboardSidebarProps, 'showTimeClock' | 'onAddTask' | 'onOpenSchedule' | 'onOpenTimeClock'>) {
  return (
...
export default function PersonalDashboardSidebar(props: PersonalDashboardSidebarProps) {
  const { selectedDate, upcoming, activity, showTimeClock, onSelectDate, onOpenJob, onOpenTask, onAddTask, onOpenSchedule, onOpenTimeClock } = props;
  return (
    <aside className="space-y-4">
      <MiniCalendarWidget selectedDate={selectedDate} onSelectDate={onSelectDate} />
      <UpcomingScheduleWidget upcoming={upcoming} onOpenJob={onOpenJob} onOpenTask={onOpenTask} />
      <RecentActivityWidget activity={activity} />
      <QuickActionsWidget showTimeClock={showTimeClock} onAddTask={onAddTask} onOpenSchedule={onOpenSchedule} onOpenTimeClock={onOpenTimeClock} />
    </aside>
  );
}
