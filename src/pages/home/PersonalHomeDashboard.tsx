...
export default function PersonalHomeDashboard({ currentUserId, currentUserName, currentUserEmail, currentUserRole, onOpenSchedule, onOpenTimeClock }: PersonalHomeDashboardProps) {
  ...
  const rootTasks = useMemo(() => getRootTasks(personalTasks), [personalTasks]);
  ...
  const filteredTasks = useMemo(() => filterTasksByRange(rootTasks, taskFilter, now).filter((task) => taskFilter !== 'today' || !dashboardPreferences.dismissedTodayTaskIds.includes(task.id)).slice().sort((left, right) => {
  ...
  }), [dashboardPreferences.dismissedTodayTaskIds, rootTasks, taskFilter]);
  const summary = useMemo(() => getTaskSummary(rootTasks, now), [rootTasks]);
  ...
  const createTask = async (input: { title: string; dueDate?: string; priority: TaskPriority; taskTabId?: string; parentTaskId?: string }) => {
  ...
  const toggleTask = async (task: Task) => {
    if (!task.parentTaskId && task.status === 'open' && personalTasks.some((item) => item.parentTaskId === task.id && item.status !== 'completed')) {
      emitAppToast({ tone: 'error', message: 'Complete all subtasks before completing this task.' });
      return;
    }
  ...
  const widgetDefinitions: HomeWidgetDefinition[] = [
    { id: 'due-today', title: 'Due Today', description: 'Tasks due today and high-priority attention.', category: 'Personal', content: <StatCard label="Due Today" value={summary.dueToday} sub={summary.highPriorityDueToday ? `${summary.highPriorityDueToday} high priority` : 'No high-priority tasks'} icon={<ListTodo />} color={summary.dueToday ? 'text-accent-700' : 'text-brand-700 dark:text-brand-100'} /> },
    { id: 'overdue', title: 'Overdue Tasks', description: 'Personal tasks that have passed their due date.', category: 'Personal', content: <StatCard label="Overdue" value={summary.overdue} sub={summary.overdue ? 'Needs attention' : 'You are caught up'} icon={<CalendarCheck2 />} color={summary.overdue ? 'text-accent-700' : 'text-brand-700 dark:text-brand-100'} /> },
    { id: 'jobs-week', title: 'Jobs This Week', description: 'Jobs assigned directly or through your crew.', category: 'Personal', content: <StatCard label="Jobs This Week" value={weekJobs.length} sub="Assigned to you or your crew" icon={<BriefcaseBusiness />} color="text-brand-700 dark:text-brand-100" /> },
    { id: 'hours-today', title: 'Hours Today', description: 'Your recorded work hours today.', category: 'Personal', content: <StatCard label="Hours Today" value={hoursToday.toFixed(1)} sub="Recorded work hours" icon={<Clock3 />} color="text-brand-700 dark:text-brand-100" /> },
    { id: 'calendar', title: 'My Calendar', description: 'Assigned work, tasks, and private calendar events.', category: 'Personal', content: <Card className="overflow-hidden rounded-lg"><PersonalCalendar jobs={personalJobs} tasks={rootTasks} customers={customers} externalEvents={externalEvents} selectedDate={selectedDate} onDateChange={setSelectedDate} onRangeChange={setCalendarRange} onOpenJob={openJob} onSelectTask={openTasks} /></Card> },
    { id: 'mini-calendar', title: 'Mini Calendar', description: 'Jump the main calendar to another day.', category: 'Personal', content: <MiniCalendarWidget selectedDate={selectedDate} onSelectDate={setSelectedDate} /> },
    { id: 'tasks', title: 'Tasks', description: 'Create, filter, and complete personal tasks.', category: 'Personal', content: <OutstandingTasks tasks={filteredTasks} allTasks={personalTasks} filter={taskFilter} customTaskTabs={dashboardPreferences.customTaskTabs} filterOrder={dashboardPreferences.taskFilterOrder} expanded={tasksExpanded} addRequest={addRequest} onFilterChange={(filter) => updateQuery({ taskFilter: filter, tasks: filter === 'all' ? searchParams.get('tasks') : 'all' })} onFilterOrderChange={dashboardPreferences.saveTaskFilterOrder} onCreateCustomTab={dashboardPreferences.createCustomTaskTab} onRenameCustomTab={dashboardPreferences.renameCustomTaskTab} onDeleteCustomTab={dashboardPreferences.deleteCustomTaskTab} onViewAll={openTasks} onAdd={createTask} onUpdate={editTask} onToggle={toggleTask} onDelete={removeTask} onDismissCompletedToday={dashboardPreferences.dismissTodayTask} /> },
    { id: 'upcoming', title: 'Upcoming Schedule', description: 'Your next jobs, tasks, and private events.', category: 'Personal', content: <UpcomingScheduleWidget upcoming={upcoming} onOpenJob={openJob} onOpenTask={openTasks} /> },
    { id: 'activity', title: 'Recent Activity', description: 'Recent changes to your work and time.', category: 'Personal', content: <RecentActivityWidget activity={activity} /> },
    { id: 'quick-actions', title: 'Quick Actions', description: 'Shortcuts to common personal workflows.', category: 'Personal', content: <QuickActionsWidget showTimeClock={Boolean(onOpenTimeClock)} onAddTask={requestAddTask} onOpenSchedule={onOpenSchedule ?? (() => navigate('/schedule'))} onOpenTimeClock={onOpenTimeClock} /> },
    ...(canViewFinancials ? [
      { id: 'clocked-in-now', title: 'Clocked In Now', description: 'Employees with an active time entry.', category: 'Operations', content: <ClockedInNowWidget items={clockedInNow} /> },
    ] satisfies HomeWidgetDefinition[] : []),
    ...(canViewFinancials ? [
      { id: 'finance-outstanding-invoices', title: 'Outstanding Invoices', description: 'Open invoice count and value from Finance.', category: 'Finance', content: <StatCard label="Outstanding Invoices" value={formatCurrency(openInvoices.reduce((total, invoice) => total + invoice.amount, 0))} sub={`${openInvoices.length} open invoice${openInvoices.length === 1 ? '' : 's'}`} icon={<Receipt />} color="text-brand-700 dark:text-brand-100" /> },
      { id: 'finance-overdue-invoices', title: 'Overdue Invoices', description: 'Invoice value currently past due.', category: 'Finance', content: <StatCard label="Overdue Invoices" value={formatCurrency(overdueInvoices.reduce((total, invoice) => total + invoice.amount, 0))} sub={`${overdueInvoices.length} past due`} icon={<FileWarning />} color={overdueInvoices.length ? 'text-accent-700' : 'text-brand-700 dark:text-brand-100'} /> },
      { id: 'finance-budget-profit', title: 'Budgeted Profit', description: 'Current-year budgeted revenue less expenses.', category: 'Finance', content: <StatCard label="Budgeted Profit" value={formatCurrency(budgetedProfit)} sub={`${currentYear} active budgets`} icon={<CircleDollarSign />} color={budgetedProfit < 0 ? 'text-accent-700' : 'text-brand-700 dark:text-brand-100'} /> },
    ] satisfies HomeWidgetDefinition[] : []),
  ];
  ...
  const dashboard = (
    <div className="space-y-5">
      ...
      <CustomizableWidgetGrid widgetIds={dashboardPreferences.widgetIds} widgetLayout={dashboardPreferences.widgetLayout} availableWidgetIds={dashboardPreferences.availableWidgetIds} definitions={widgetDefinitions} hydrated={dashboardPreferences.hydrated} onChange={dashboardPreferences.saveDashboard} onReset={dashboardPreferences.resetWidgetIds} />
    </div>
  );
