...
import { normalizeEstimateCustomSellPrice } from '../../utils/estimatePricingModel.js';
...
import { calculateServiceEconomics, calculateServiceEstimateTotals, formatServiceFrequency, resolveServiceEstimatedVisits, roundServiceMoney } from '../../utils/servicePricingModel.js';
...
export default function ServiceEstimateWorkspacePage() {
  ...
  const [resourceDraft, setResourceDraft] = useState<{ serviceId: string; line: ServiceEstimateLineItem } | null>(null);
  ...
  const selectCatalogItem = (item: EstimatePricingCatalogItem) => {
    if (!catalogService || (!item.pricingAvailable && item.pricingReadiness !== 'needs_review')) return;
    const line = calculateEstimateLineItem({ ...applyEstimatePricingToLineItem(createEmptyEstimateLineItem(catalogCategory), form.pricingBudgetId, item), costScope: 'per_visit' } as ServiceEstimateLineItem) as ServiceEstimateLineItem;
    setResourceDraft({ serviceId: catalogService.id, line });
  };
  const pricingService = pricingTarget ? services.find((service) => service.id === pricingTarget.serviceId) : null;
  const pricingLine = pricingTarget ? pricingService?.lineItems?.find((line) => line.id === pricingTarget.lineId) : null;
  ...
  return <div>
    ...
    {catalogServiceId && !resourceDraft ? <div className="fixed inset-0 z-50"><button aria-label="Close catalog" className="absolute inset-0 bg-black/50" onClick={() => setCatalogServiceId(null)} /><aside className="absolute inset-y-0 right-0 w-full max-w-xl overflow-y-auto bg-white p-5 shadow-xl dark:bg-brand-900"><div className="flex items-center justify-between"><h2 className="text-lg font-semibold">Add resource</h2><Button variant="ghost" size="sm" title="Close" onClick={() => setCatalogServiceId(null)}><X /></Button></div><div className="mt-4"><Input label="Search" value={catalogSearch} onChange={(event) => setCatalogSearch(event.target.value)} /></div><div className="mt-4 flex gap-2 overflow-x-auto">{CATEGORIES.map((category) => <Button key={category.value} size="sm" variant={catalogCategory === category.value ? 'primary' : 'secondary'} onClick={() => setCatalogCategory(category.value)}>{category.label}</Button>)}</div>{catalogError ? <p className="mt-4 text-sm text-red-600">{catalogError}</p> : null}<div className="mt-4 divide-y divide-brand-100 dark:divide-brand-700">{catalogItems.map((item) => <button key={`${item.budgetItemId}-${item.materialCatalogItemId}-${item.divisionId}`} type="button" disabled={!item.pricingAvailable && item.pricingReadiness !== 'needs_review'} onClick={() => selectCatalogItem(item)} className="flex w-full items-center justify-between gap-4 py-4 text-left disabled:opacity-40"><div><p className="font-medium">{item.name}</p><p className="text-xs text-brand-400">{item.divisionName ?? 'All divisions'} · {item.description}</p></div><span className="shrink-0 font-semibold">{item.sellRate ? `${formatCurrency(item.sellRate)}/${item.unit}` : 'Review'}</span></button>)}</div></aside></div> : null}
    {resourceDraft ? <EstimateLinePricingEditor lineItem={resourceDraft.line} initialMode="price" editServiceUsage estimatedVisits={resolveServiceEstimatedVisits(catalogService ?? {})} title="Add Resource" saveLabel="Add Resource" onChange={(line: EstimateLineItem) => { const service = services.find((item) => item.id === resourceDraft.serviceId); if (service) updateService(service.id, { lineItems: [...(service.lineItems ?? []), line as ServiceEstimateLineItem] }); setResourceDraft(null); setCatalogServiceId(null); }} onClose={() => { setResourceDraft(null); setCatalogServiceId(null); }} /> : null}
    {pricingLine && pricingTarget ? <EstimateLinePricingEditor lineItem={pricingLine} initialMode="price" editServiceUsage estimatedVisits={resolveServiceEstimatedVisits(pricingService ?? {})} title="Edit Resource" saveLabel="Save Resource" onChange={(line: EstimateLineItem) => updateLine(pricingTarget.serviceId, pricingTarget.lineId, line)} onClose={() => setPricingTarget(null)} /> : null}
  </div>;
}

function ServiceEditor({ service, index, divisions, updateService, remove, openCatalog, openPricing }: { service: EstimateService; index: number; divisions: Array<{ id: string; name: string }>; updateService: (id: string, patch: Partial<EstimateService>) => void; remove: () => void; openCatalog: () => void; openPricing: (lineId: string) => void }) {
  const economics = calculateServiceEconomics(service);
  const lines = service.lineItems ?? [];
  const recommendedContractTotal = economics.calculatedServiceValue;
  const customerContractTotal = service.contractPricing?.customContractPrice ?? recommendedContractTotal;
  const hasCustomContractTotal = service.contractPricing?.customContractPrice != null
    && roundServiceMoney(service.contractPricing.customContractPrice) !== roundServiceMoney(recommendedContractTotal);
  return <Card className="p-5"><div className="mb-4 flex items-center justify-between"><div><h3 className="font-semibold">{service.name || `Service ${index + 1}`}</h3><p className="text-xs text-brand-400">{formatServiceFrequency(service)} · {economics.estimatedVisits} estimated visits</p></div><Button variant="ghost" size="sm" title="Remove Service" onClick={remove}><Trash2 /></Button></div><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Input label="Service name" value={service.name} onChange={(event) => updateService(service.id, { name: event.target.value })} /><Select label="Division" value={service.divisionId ?? ''} onChange={(event) => updateService(service.id, { divisionId: event.target.value || undefined })}><option value="">No division</option>{divisions.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</Select><Select label="Schedule" value={service.scheduleType} onChange={(event) => updateService(service.id, { scheduleType: event.target.value as EstimateService['scheduleType'] })}><option value="recurring">Recurring</option><option value="one_time">One time</option><option value="as_needed">As needed</option></Select><Select label="Billing" value={service.billingType} onChange={(event) => updateService(service.id, { billingType: event.target.value as EstimateService['billingType'] })}><option value="contract">Contract</option><option value="per_visit">Per visit</option><option value="time_and_material">Time & material</option></Select><Input label="Start date" type="date" value={service.startDate ?? ''} onChange={(event) => updateService(service.id, { startDate: event.target.value || undefined })} /><Input label="End date" type="date" value={service.endDate ?? ''} onChange={(event) => updateService(service.id, { endDate: event.target.value || undefined })} /><Input label="Estimated visits" type="number" min={0} value={service.scheduleType === 'one_time' ? 1 : service.estimatedVisits ?? calculateSuggestedServiceVisits(service) ?? 0} onChange={(event) => updateService(service.id, { estimatedVisits: Number(event.target.value) })} />{service.scheduleType === 'recurring' ? <Input label="Every (weeks)" type="number" min={1} value={service.frequency?.interval ?? 1} onChange={(event) => updateService(service.id, { frequency: { interval: Number(event.target.value), unit: 'week' } })} /> : null}</div><div className="mt-4"><TextArea label="Customer scope" value={service.description} onChange={(event) => updateService(service.id, { description: event.target.value })} /></div>
    <div className="mt-5 flex items-center justify-between border-t border-brand-100 pt-4 dark:border-brand-700"><div><p className="font-semibold">Resources</p><p className="text-xs text-brand-400">Loaded cost {formatCurrency(economics.estimatedCost)} · Recommended {formatCurrency(recommendedContractTotal)}</p></div><Button variant="secondary" size="sm" onClick={openCatalog}><Plus /> Add resource</Button></div><div className="mt-3 overflow-x-auto"><table className="w-full min-w-[720px] text-sm"><thead className="text-left text-xs text-brand-400"><tr><th className="py-2">Resource</th><th>Qty</th><th>Unit</th><th>Applied</th><th>Sell rate</th><th /></tr></thead><tbody>{lines.map((line) => <tr key={line.id} className="border-t border-brand-100 dark:border-brand-700"><td className="py-2 font-medium">{line.itemName}</td><td>{line.quantity}</td><td>{line.unit}</td><td>{line.costScope === 'per_visit' ? 'Per visit' : 'Once'}</td><td>{formatCurrency(line.sellPrice)}/{line.unit}{line.estimateCustomSellPrice != null && normalizeEstimateCustomSellPrice(line.estimateCustomSellPrice, line.calculatedRateAtEstimate ?? line.sellPrice) !== null ? <span className="ml-2 text-xs font-medium text-accent-700">Custom price</span> : null}</td><td><div className="flex justify-end gap-1"><Button variant="ghost" size="sm" title="Edit resource" onClick={() => openPricing(line.id)}><SlidersHorizontal /></Button><Button variant="ghost" size="sm" title="Remove resource" onClick={() => updateService(service.id, { lineItems: lines.filter((item) => item.id !== line.id) })}><Trash2 /></Button></div></td></tr>)}</tbody></table></div>
    <div className="mt-5 grid gap-4 border-t border-brand-100 pt-4 sm:grid-cols-2 dark:border-brand-700">{service.billingType === 'contract' ? <><div><Input label="Recommended contract total" type="number" currency value={recommendedContractTotal} disabled /><p className="mt-1 text-xs text-brand-400">Calculated from resources, overhead and profit</p><p className="mt-1 text-xs text-brand-400">{economics.estimatedVisits} estimated visits</p></div><div><Input label="Customer contract total" type="number" min={0} currency value={customerContractTotal} onChange={(event) => { const next = Math.max(0, parseNumericInputValue(event.target.value)); updateService(service.id, { contractPricing: { customContractPrice: roundServiceMoney(next) === roundServiceMoney(recommendedContractTotal) ? null : next } }); }} /><p className="mt-1 text-xs text-brand-400">What you will charge the customer for this service</p>{hasCustomContractTotal ? <div className="mt-2 flex items-center justify-between gap-3"><span className="text-xs text-brand-500">Recommended {formatCurrency(recommendedContractTotal)}</span><button type="button" className="text-xs font-semibold text-brand-600 hover:text-brand-800 dark:text-brand-200" onClick={() => updateService(service.id, { contractPricing: { customContractPrice: null } })}>Reset to recommended</button></div> : null}</div></> : service.billingType === 'per_visit' ? <><Input label="Price per visit" type="number" min={0} currency value={service.perVisitPricing?.customPricePerVisit ?? economics.recommendedPricePerVisit} onChange={(event) => updateService(service.id, { perVisitPricing: { ...service.perVisitPricing, customPricePerVisit: parseNumericInputValue(event.target.value) } })} /><Input label="One-time charge" type="number" min={0} currency value={service.perVisitPricing?.oneTimeCharge ?? economics.servicePeriodEffectiveSell} onChange={(event) => updateService(service.id, { perVisitPricing: { ...service.perVisitPricing, oneTimeCharge: parseNumericInputValue(event.target.value) } })} /><Button variant="secondary" onClick={() => updateService(service.id, { perVisitPricing: { customPricePerVisit: null, oneTimeCharge: null } })}>Reset recommended</Button></> : <TextArea label="T&M customer terms" value={service.timeAndMaterialPricing?.notes ?? ''} onChange={(event) => updateService(service.id, { timeAndMaterialPricing: { notes: event.target.value } })} />}</div>
  </Card>;
}
