...
export default function ServiceEstimateWorkspacePage() {
  ...
  return <div>
    ...
    {resourceDraft ? <EstimateLinePricingEditor lineItem={resourceDraft.line} initialMode="price" editServiceUsage title="Add Resource" saveLabel="Add Resource" onChange={(line: EstimateLineItem) => { const service = services.find((item) => item.id === resourceDraft.serviceId); if (service) updateService(service.id, { lineItems: [...(service.lineItems ?? []), line as ServiceEstimateLineItem] }); setResourceDraft(null); setCatalogServiceId(null); }} onClose={() => { setResourceDraft(null); setCatalogServiceId(null); }} /> : null}
    {pricingLine && pricingTarget ? <EstimateLinePricingEditor lineItem={pricingLine} initialMode="price" editServiceUsage title="Edit Resource" saveLabel="Save Resource" onChange={(line: EstimateLineItem) => updateLine(pricingTarget.serviceId, pricingTarget.lineId, line)} onClose={() => setPricingTarget(null)} /> : null}
  </div>;
}
