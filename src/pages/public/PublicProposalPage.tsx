    ...
    <main className="min-h-screen bg-[#f2f1ec] px-3 py-4 text-stone-800 sm:px-6 sm:py-8">
      <article className="mx-auto max-w-4xl bg-white shadow-[0_18px_60px_rgba(40,45,35,0.12)]">
        ...
        <div className="px-5 py-7 sm:px-10 sm:py-9">
          <section className="grid gap-7 border-b border-stone-200 pb-7 sm:grid-cols-2"><div><p className="text-xs font-bold uppercase text-emerald-800">Prepared for</p><p className="mt-2 font-serif text-xl text-stone-950">{snapshot.customer.displayName}</p><p className="mt-1 whitespace-pre-line text-sm text-stone-600">{[snapshot.customer.billingAddress, snapshot.customer.email, snapshot.customer.phone].filter(Boolean).join('\n')}</p></div><div><p className="text-xs font-bold uppercase text-emerald-800">Project</p><p className="mt-2 font-serif text-xl text-stone-950">{snapshot.proposal.title}</p><p className="mt-1 text-sm text-stone-600">{snapshot.proposal.projectAddress}</p></div></section>
          <section className="py-8"><h2 className="font-serif text-2xl text-stone-950">Scope of Work</h2><div className="mt-5 space-y-6">{snapshot.workAreas.map((area) => <div key={area.name}><h3 className="font-serif text-lg font-semibold text-emerald-900">{area.name}</h3><ul className="mt-2 space-y-2 text-sm leading-6 text-stone-700">{area.scopeLines.map((line) => <li key={line} className="flex gap-3"><span className="mt-2 h-1.5 w-1.5 shrink-0 bg-emerald-700" />{line}</li>)}</ul></div>)}</div></section>
          <section className="border-y border-stone-200 py-7"><div className="ml-auto max-w-sm space-y-2 text-sm"><div className="flex justify-between"><span>Subtotal</span><span>{money(snapshot.proposal.subtotal)}</span></div><div className="flex justify-between"><span>{snapshot.proposal.taxLabel} ({snapshot.proposal.taxRate}%)</span><span>{money(snapshot.proposal.taxAmount)}</span></div><div className="flex justify-between border-t border-stone-300 pt-3 font-serif text-xl font-semibold text-emerald-900"><span>Proposal Total</span><span>{money(snapshot.proposal.total)}</span></div></div></section>
