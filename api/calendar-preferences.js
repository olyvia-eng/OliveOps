...
const defaults = { view: 'week', colourBy: 'crew', showGoogleEvents: true };
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!['month', 'week', 'day'].includes(next.view) || !['crew', 'division', 'status'].includes(next.colourBy) || typeof next.showGoogleEvents !== 'boolean') {
    ...
    const preferences = await saveCalendarPreferencesForUser({
      ...
      userId: session.id,
      preferences: { view: next.view, colourBy: next.colourBy, showGoogleEvents: next.showGoogleEvents },
    });
